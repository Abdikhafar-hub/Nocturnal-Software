...
export default function AllBlogs() {
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6 lg:px-8 bg-background blog-section w-full overflow-x-hidden">
      <div ref={ref} className="max-w-7xl mx-auto opacity-0 translate-y-10 transition-all duration-700 blog-container w-full">
        <div className="text-center mb-8 sm:mb-12 md:mb-16 blog-header">
          <h1 className="blog-title font-bold mb-3 sm:mb-4 text-primary text-3xl sm:text-4xl md:text-5xl">All Blog Posts</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 blog-grid">
          {getSortedBlogPosts().map((post) => (
            ...
            >
              ...
              <Link href={`/blog/${post.id}`}>
                <div className="relative blog-image-wrapper overflow-hidden bg-gradient-to-br from-primary/10 to-accent/10 cursor-pointer h-48 sm:h-56">
                  <img
                    src={post.image}
                    alt={post.title}
              ...
              {/* Content */}
              <div className="p-4 sm:p-6 blog-content">
                <Link href={`/blog/${post.id}`}>
